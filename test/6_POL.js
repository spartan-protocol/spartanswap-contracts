...
contract('PROTOCOL OWNED LIQUIDITY', function (accounts) {
...
function constructor(accounts) {
    ...
    it("constructor events", async () => {
        ...
        reserve = await RESERVE.new(sparta.address, wbnb.address) // deploy reserve 
...
async function curatePools() {
    it("Curate POOls", async () => {
        ...
        await reserve.setParams(token1.address, { from: acc0 });
        await reserve.flipEmissions();
        await reserve.flipPol();
...
async function print(){
    it(`Console Reserve`, async () =>{
      let bal = _.BN2Str(await sparta.balanceOf(reserve.address))
      let balB = _.BN2Str(await poolBUSD.balanceOf(reserve.address))
      let balBNB = _.BN2Str(await poolBNB.balanceOf(reserve.address))
     console.log(balB);
     console.log(bal);
     console.log(balBNB);
    console.log(await reserve.globalFreeze())
       
    })
}
async function print2(){
    it(`Console setPara`, async () =>{
        await reserve.setParams(_.BNB, { from: acc0 });
