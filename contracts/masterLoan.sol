...
contract SpartanLoan {
    ...
    function addCollateralForMember(uint inputLPToken, address lpToken, address member, address loanAsset) public payable returns (uint loanedAssetAmount) {
        require(iPOOLFACTORY(_DAO().POOLFACTORY()).isCuratedPool(lpToken) == true, "LP tokens must be from Curated pools");
        address _loanPool = iPOOLFACTORY(_DAO().POOLFACTORY()).getPool(loanAsset);
        require(iPOOLFACTORY(_DAO().POOLFACTORY()).isCuratedPool(_loanPool) == true, "Asset Loan from Curated pools");
